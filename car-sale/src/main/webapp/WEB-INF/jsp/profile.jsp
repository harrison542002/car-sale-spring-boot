		...
		<div class="p-4" id="profile" data-profileId="${profile.profile_id}">
			<div class="flex justify-center">
				<img class="rounded-full w-20 shadow-lg"
					src="<c:url value='/img/profile.jpeg'/>" alt="profile image">
			</div>
			<div class="profile-info">
				<div class="mt-5 pl-2">
					<p class="text-xl font-bold">
					<span id="firstNamee">${profile.firstName}</span>
					<span id="lastNamee">${profile.lastName}</span>
					</p>
			...
			<div class="text-l text-center mt-2 lg:block sm:flex">
                ...
                hover:text-indigo-500"
					href="/profile?status=PENDING"> Pending Cars </a>
				<a 
					class="lg:p-4 lg:border block lnav p-3
                <c:if test="${status == 'BOOKING'}">
                 ...
                 hover:text-indigo-500"
                 href="/profile?status=BOOKING">
					Review Bookings 
				</a>
			</div>
		</div>
		<div class="col-span-4 border-gray-300 border-gray-900 border-l-4 rounded-md">
			<div class="flex justify-end m-2">
				<a href="<c:url value='/cars/postCarForm'/>">
					<button
						class="bg-indigo-500 p-2 text-lg font-semibold text-gray-100 rounded-lg">
						Post New Car
					</button>
				</a>
			</div>
			...
			<div class="carLists overflow-y-auto h-[30rem] rounded-bl-lg">
				<c:forEach items="${cars}" var="car" varStatus="state">
					<div class="p-3 hover:text-gray-500 hover:bg-gray-100
							...
							<div>
								...
								<div class="grid grid-cols-2 mt-4">
									<div class="subTitle font-bold">
										...
										<p class="font-semibold">Regsitration Year</p>
								...
								<div class="flex">
									<p> Booking Status : </p>
									<div class=" ml-2
									<c:if test="${appointments.get(state.index).status == 'PENDING'}">
									text-gray-400">
										<i class="fa-solid fa-clock"></i> Pending
									</div>
									</c:if>
									<c:if test="${appointments.get(state.index).status == 'APPROVE'}">
									text-green-400">
										<i class="fa-solid fa-circle-check"></i> Approved
									</div>
									</c:if>
